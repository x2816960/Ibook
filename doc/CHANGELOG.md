...
### Bug 修复
- **修复任务改为“无限期”后截止时间未清空的问题**
  - 问题：任务编辑时勾选“无限期”后，接口返回中仍保留旧 `due_date`，导致前端显示和数据状态不一致
  - 修复：
    - 更新任务时，`is_indefinite=true` 会同步清空 `due_date`
    - 今日到期/已过期统计与筛选统一排除无限期任务
  - 修改文件：
    - `backend/app/services/task_service.py`

- **修复拖拽排序会污染全局顺序的问题**
  - 问题：在当前页或筛选结果中拖拽排序时，提交的任务会被重排到全局最前，并产生重复 `sort_order`
  - 修复：
    - 排序接口仅调整本次提交任务在当前结果中的相对顺序
    - 服务端重新规整全局 `sort_order`，避免重复序号
  - 修改文件：
    - `backend/app/services/task_service.py`

### 优化
- **移除任务编辑表单中的状态字段**
  - 原因：任务状态应通过独立状态切换入口修改，避免和后端接口语义不一致
  - 修改文件：
